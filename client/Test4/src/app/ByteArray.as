package app  {
	...
	public class ByteArray {
		...
		public function outputBytes(output:Byte, offset:uint = 0, len:uint = 0):void{
			if(len == 0){
				len = this.length - offset;
			}
			for(var i:int=offset; i<len; i++){
				output.writeByte(this.__get(i));
			}
		}
		...
		public function readUnsignedInt ():int 
		{
			...
			this._position_ += 4;
			return int(uInt);//add by ch.ji 解决读取整数时读到负整数的问题
		}
			...
			while (this._position_ < max) {
				...
				if (c < 0x80) {
				   ...
				   if (c !=0)
				   {
					...
					c4 = _data_.getUint8 (this._position_++);
					value += String.fromCharCode (((c & 0x0F) << 18) | ((c2 & 0x7F) << 12) | ((c3 << 6) & 0x7F) | (c4 & 0x7F));
				}
			}	
			return value;
...
//			var flag = 0;
//			var buf = _data_;
//			for (pos = this._position_; pos < len;){
//				flag= buf.getUint8(pos);
//				if ((flag >>>7) === 0 ) {
//					str+= String.fromCharCode(buf.getUint8(pos));
//					pos += 1;
//					
//				}
//				else if ((flag &0xFC) === 0xFC ){
//					unicode = (buf.getUint8(pos) & 0x3) << 30;
//					unicode |= (buf.getUint8(pos+1) & 0x3F) << 24; 
//					unicode |= (buf.getUint8(pos+2) & 0x3F) << 18; 
//					unicode |= (buf.getUint8(pos+3) & 0x3F) << 12; 
//					unicode |= (buf.getUint8(pos+4) & 0x3F) << 6;
//					unicode |= (buf.getUint8(pos+5) & 0x3F);
//					str+= String.fromCharCode(unicode) ;
//					pos += 6;
//					
//				}else if ((flag &0xF8) === 0xF8 ){
//					unicode = (buf.getUint8(pos) & 0x7) << 24;
//					unicode |= (buf.getUint8(pos+1) & 0x3F) << 18; 
//					unicode |= (buf.getUint8(pos+2) & 0x3F) << 12; 
//					unicode |= (buf.getUint8(pos+3) & 0x3F) << 6;
//					unicode |= (buf.getUint8(pos+4) & 0x3F);
//					str+= String.fromCharCode(unicode) ;
//					pos += 5;
//					
//				}
//				else if ((flag &0xF0) === 0xF0 ){
//					unicode = (buf.getUint8(pos) & 0xF) << 18;
//					unicode |= (buf.getUint8(pos+1) & 0x3F) << 12; 
//					unicode |= (buf.getUint8(pos+2) & 0x3F) << 6;
//					unicode |= (buf.getUint8(pos+3) & 0x3F);
//					str+= String.fromCharCode(unicode) ;
//					pos += 4;
//					
//				}
...
//					unicode = (buf.getUint8(pos) & 0x1F) << 12;;
//					unicode |= (buf.getUint8(pos+1) & 0x3F) << 6;
//					unicode |= (buf.getUint8(pos+2) & 0x3F);
//					str+= String.fromCharCode(unicode) ;
//					pos += 3;
//					
//				}
//				else if ((flag &0xC0) === 0xC0 ){ //110
//					unicode = (buf.getUint8(pos) & 0x3F) << 6;
//					unicode |= (buf.getUint8(pos+1) & 0x3F);
//					str+= String.fromCharCode(unicode) ;
//					pos += 2;
//					
//				}
